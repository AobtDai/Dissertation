...
for model_name in model_name_list:
    ...
    #     continue
    img_path_original = "./dataset/translated-LLaVA-Instruct-150K/llava-imgs/filtered-llava-images" 
    img_path_Complex = "./dataset/MyDataset/Complex"
    img_path_GaussianBlur = "./dataset/MyDataset/GaussianBlur"
    img_path_GaussianNoise = "./dataset/MyDataset/GaussianNoise"
    img_path_list = [img_path_original, img_path_Complex, img_path_GaussianBlur, img_path_GaussianNoise]
    with open("./dataset/MyDataset/PromptEval.json", 'r') as file:
        data0 = json.load(file)
    out_file = open("./output/image/{}.json".format(model_name), 'w')
    tot_data = []
    ...
    if model_name=="llava-v1.6-mistral-7b":
        ...
        for it, _data in tqdm(enumerate(data0), total=len(data0)):
            ...
            for _img_path in img_path_list:
                img_path = os.path.join(_img_path, _data["image"])
                image = Image.open(img_path)
                prompt = "[INST] <image>\n{} [/INST]".format(_data["questions"][0])
        ...
        for it, _data in tqdm(enumerate(data0), total=len(data0)):
            ...
            for _img_path in img_path_list:
                ...
                prompt = "USER: <image>\n{} ASSISTANT:".format(_data["questions"][0])
                inputs = processor(prompt, image, return_tensors="pt").to("cuda:2")
                output = model.generate(**inputs, max_new_tokens=100)
